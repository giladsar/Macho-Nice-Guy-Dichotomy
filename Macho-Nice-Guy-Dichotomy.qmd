...
# DDR
I will next construct the Macho and Nice Guy semantic contrasts (DDRs). To do so, I will implement a helper function that iterates over the word-embedding models by decade. When the nudge option is set to TRUE, the function will first construct a masculinity-nudge axis. It will then compute the DDRs for each guy-type construct.
...
# --- 
decade_scores_helper <- lapply(ENGall_model, function(embeddings){
...
for (i in seq_along(ENGall_model)) {
  ...
  Macho_DDR_injected_by_decade[[i]] <- predict(ENGall_model[[i]], macho_guy_words_injected)|> 
...
for (i in seq_along(ENGall_model)) {
  ...
  Nice_DDR_injected_by_decade[[i]] <- predict(ENGall_model[[i]], nice_guy_words_injected)|> 
  average_embedding()
}
...
for (i in seq_along(ENGall_model)) {
  ...
  Macho_DDR_nudge_by_decade[[i]] <- predict(ENGall_model[[i]], macho_guy_words)|>
  average_embedding() |>
...
for (i in seq_along(ENGall_model)) {
  ...
  Nice_DDR_nudge_by_decade[[i]] <- predict(ENGall_model[[i]], nice_guy_words)|>
  average_embedding() |>
...
for (i in seq_along(ENGall_model)) {
  ...
  warriner_with_COSINE_from_DDR[[i]] <- warriner_dfm|>
    ...
    textstat_embedding(ENGall_model[[i]])  |> 
...
nice_table_all_words_fig<- kable(
...
macho_table_all_words_fig<- kable(
...
for (i in seq_along(ENGall_model)) {
  ...
  Attractiveness_DDR_injected_by_decade[[i]] <- predict(ENGall_model[[i]], attractiveness_words_injected)|> 
    average_embedding()
}
...
for (i in seq_along(ENGall_model)) {
  Attractiveness_DDR_nudge_by_decade[[i]] <- predict(ENGall_model[[i]], attractiveness_words)|>
  average_embedding() |>
    masculinity_nudge(E = ENGall_model[[i]])
}
...
inject_attr_plot <- ggplot(cosine_attract_inject_df, aes(x = decade)) +
 ...
 combied_plot<- nudge_attr_plot + inject_attr_plot +
...
```{r StatisticalTestAttractiveness}
# Nudge t-test  
