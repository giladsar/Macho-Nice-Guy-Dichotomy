...
cohens_d_pooled <- function(x, y) {
  diff <- x - y
  pooled_sd <- sqrt((var(x, na.rm = TRUE) + var(y, na.rm = TRUE)) / 2)
  diff / pooled_sd
}
compare_DDRs<- function(df) {
  ...
  # delta (raw): 
  delta_mean_nudge <- mean(df$macho_nudge,na.rm = T) - mean(df$nice_nudge,na.rm = T) 
  delta_mean_inject <- mean(df$macho_inject,na.rm = T) - mean(df$nice_inject,na.rm = T) 
  ...
  df |> 
    mutate(
      ...
      delta_nudge_mean = delta_mean_nudge,
      nudge_d = cohens_d_pooled(macho_nudge,nice_nudge),
      # Inject
      delta_inject = macho_inject - nice_inject,
      delta_inject_mean = delta_mean_inject,
      inject_d = cohens_d_pooled(macho_inject,nice_inject),
    )
  
  
  # # T-test results for inject
  # tt_inject = list(t.test(delta_inject,
  #                         mu = group_mean_inject, # this vector is constant so I choice the first element because irt doesn't matter
  #                         alternative = "two.sided")),
  # p_inject = tt_inject[[1]]$p.value,
  # t_inject = tt_inject[[1]]$statistic,
  # ci_low_inject = tt_inject[[1]]$conf.int[1],
  # ci_high_inject = tt_inject[[1]]$conf.int[2]
  # ) |>
  # select(-tt_nudge, -tt_inject)  
}
