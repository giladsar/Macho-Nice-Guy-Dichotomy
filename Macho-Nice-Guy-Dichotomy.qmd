...
# a sanity check has reviled that the those words recievned the higher cos sim (DUH!)
warriner_COSINE_DDR_omit_mch_nce<- list()
for (i in seq_along(warriner_with_COSINE_from_DDR)) {
  warriner_COSINE_DDR_omit_mch_nce[[i]]<- warriner_with_COSINE_from_DDR[[i]] |> 
    filter(!word %in% c(macho_guy_words, nice_guy_words))
}
names(warriner_COSINE_DDR_omit_mch_nce) <- decades
