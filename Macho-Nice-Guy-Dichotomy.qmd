...
summ_t_inject <- summ_t_wtnin_dcds |>
  select(-contains("_nudge"))

summ_t_nudge <- summ_t_wtnin_dcds |>
...
words_t_summ<- raw_long  |> 
  #removing na  by delta_nudge 
  
  filter(!is.na(delta_nudge)) |>
  
  # This time I'll group by WORD 
...
# NOTICE that in raw_long we omitted the traits that were used to create our DDRs - so we shouldn't see any wo these words (macho_guy_words and nice_guy_words) in the trait list


traits_t_summ<- raw_long  |> 
  #removing na  by delta_nudge 
  
  filter(!is.na(delta_nudge)) |> 
  
  # filtering to have the analysis conducted soley on the traits 
  filter(doc_id %in% (traits_warriner |>
                        select(id) |>
                        pull())) |>
